...
class HandoffService {
  ...
  async generate(storyId) {
    const story = this.db.query(`SELECT id, title, file_path, linked_plan_path, size, status
         FROM stories WHERE id = ?`).get(storyId);
    if (!story)
      return null;
...
`);
    ...
    this.db.run(`INSERT INTO handoffs (story_id, file_path, generated_at) VALUES (?, ?, ?)`, [storyId, filePath, Date.now()]);
...
`);
  ...
  list() {
    return this.db.query(`SELECT id, story_id, file_path, generated_at FROM handoffs ORDER BY generated_at DESC`).all();
  }
}
...
async function dispatchEvent(event, db, bus) {
...
async function handleHookEvent(_eventName, body, db, bus, watcher) {
  let event;
  try {
