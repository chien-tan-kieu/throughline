...
describe("StandupService", () => {
  ...
  test("shipped includes story done in prior calendar day window", () => {
    ...
    const may16noon = new Date("2026-05-16T12:00:00Z").getTime();
  ...
  test("shipped excludes story done on the requested date itself (wrong day)", () => {
    const may17noon = new Date("2026-05-17T12:00:00Z").getTime();
  ...
  test("shipped excludes story done two days ago", () => {
    const may15noon = new Date("2026-05-15T12:00:00Z").getTime();
  ...
  test("blockers: fewer than 3 failures = no blocker", () => {
    const may16start = new Date("2026-05-16T00:00:00Z").getTime();
    const sessionStart = may16start + 3600_000; // 1 hour after day start
    db.run(
      `INSERT INTO sessions (id, cwd, permission_mode, started_at, status, active_story_id)
       VALUES (?, '/tmp', 'auto', ?, 'active', ?)`,
      ["sess-1", sessionStart, "US-005"],
    );
    for (let i = 0; i < 2; i++) {
      db.run(
        `INSERT INTO events (session_id, event_name, payload_json, ts)
         VALUES (?, 'PostToolUseFailure', ?, ?)`,
        ["sess-1", JSON.stringify({ tool_name: "Edit" }), sessionStart + i * 1000],
      );
    }
    const result = svc.generate("2026-05-17");
    expect(result.blockers).toHaveLength(0);
  ...
  test("blockers: 3 or more failures for same tool in same session = blocker", () => {
    ...
    db.run(
      `INSERT INTO sessions (id, cwd, permission_mode, started_at, status, active_story_id)
       VALUES (?, '/tmp', 'auto', ?, 'active', ?)`,
      ["sess-2", sessionStart, "US-005"],
    );
    for (let i = 0; i < 3; i++) {
      db.run(
        `INSERT INTO events (session_id, event_name, payload_json, ts)
         VALUES (?, 'PostToolUseFailure', ?, ?)`,
        ["sess-2", JSON.stringify({ tool_name: "Edit" }), sessionStart + i * 1000],
      );
    }
    const result = svc.generate("2026-05-17");
  ...
  test("blockers: events older than 24h are excluded", () => {
    ...
    const may16start = new Date("2026-05-16T00:00:00Z").getTime();
    const may15start = may16start - 86_400_000; // 25 hours before May 16 00:00
    const sessionStart = may15start - 3600_000; // 1 hour before cutoff
    db.run(
      `INSERT INTO sessions (id, cwd, permission_mode, started_at, status, active_story_id)
       VALUES (?, '/tmp', 'auto', ?, 'active', ?)`,
      ["sess-3", sessionStart, "US-006"],
    );
    for (let i = 0; i < 5; i++) {
      db.run(
        `INSERT INTO events (session_id, event_name, payload_json, ts)
         VALUES (?, 'PostToolUseFailure', ?, ?)`,
        ["sess-3", JSON.stringify({ tool_name: "Bash" }), sessionStart + i * 1000],
      );
    }
    const result = svc.generate("2026-05-17");
    expect(result.blockers).toHaveLength(0);
